...
const EMAIL_CONFIG = {
    ...
    host: (process.env.EMAIL_USER || '').indexOf('gmail') !== -1 ? 'smtp.gmail.com' : 'smtp.qq.com',
    port: 587
};
...
async function sendEmail(toEmail, subject, htmlContent, textContent, templateParams) {
...
async function sendOrderEmail(order, status, toEmail) {
    ...
    if (order.products) {
        ...
        for (var i = 0; i < order.products.length; i++) {
            ...
            productsHtmlList.push('<tr><td style="padding: 15px; border-bottom: 1px solid #eee; text-align: left;">' + (p.name || '商品') + '</td><td style="padding: 15px; border-bottom: 1px solid #eee; text-align: center;">' + (p.quantity || 1) + '</td><td style="padding: 15px; border-bottom: 1px solid #eee; text-align: right;">¥' + ((p.price || 0).toFixed(2)) + '</td></tr>');
        }
        productsList = productsHtmlList.join('');
    }

    const htmlContent = '<div style="font-family: -apple-system, BlinkMacSystemFont, \'Segoe UI\', Roboto, Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;"><div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; text-align: center; border-radius: 12px 12px 0 0;"><h1 style="color: white; margin: 0; font-size: 24px;">' + statusIcon + ' KDX商城</h1><p style="color: rgba(255,255,255,0.9); margin: 10px 0 0 0;">订单状态通知</p></div><div style="background: #f8f9fa; padding: 30px; border-radius: 0 0 12px 12px;"><div style="background: ' + statusColor + '15; padding: 20px; border-left: 4px solid ' + statusColor + '; border-radius: 8px; margin-bottom: 25px;"><p style="color: ' + statusColor + '; margin: 0; font-weight: 600; font-size: 16px;">' + statusText + '</p></div><div style="background: white; padding: 25px; border-radius: 10px;"><p style="margin: 0 0 15px 0; color: #333;"><strong>订单号：</strong> <span style="color: #667eea;">' + (order.id || '未知') + '</span></p><p style="margin: 0 0 25px 0; color: #666; font-size: 14px;"><strong>下单时间：</strong> ' + new Date(order.createdAt || Date.now()).toLocaleString('zh-CN') + '</p><h3 style="margin: 0 0 15px 0; color: #333; border-bottom: 2px solid #eee; padding-bottom: 10px;">📋 商品清单</h3><table style="width: 100%; border-collapse: collapse;"><thead><tr style="background: #f8f9fa;"><th style="padding: 12px; text-align: left; color: #666;">商品</th><th style="padding: 12px; text-align: center; color: #666;">数量</th><th style="padding: 12px; text-align: right; color: #666;">单价</th></tr></thead><tbody>' + productsList + '</tbody></table><div style="margin-top: 25px; padding-top: 20px; border-top: 2px solid #eee; text-align: right;"><p style="margin: 0; color: #666;">订单总额：<span style="color: #ef4444; font-size: 28px; font-weight: bold; margin-left: 10px;">¥' + ((order.total || 0).toFixed(2)) + '</span></p></div></div><div style="margin-top: 25px; text-align: center;"><p style="color: #999; font-size: 14px; margin: 0;">如有问题，请随时联系客服</p><p style="color: #ccc; font-size: 12px; margin-top: 10px;">这是一封自动发送的邮件，请勿直接回复</p></div></div></div>';
...
async function sendRefundEmail(refund, toEmail) {
    const htmlContent = '<div style="font-family: -apple-system, BlinkMacSystemFont, \'Segoe UI\', Roboto, Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;"><div style="background: linear-gradient(135deg, #f59e0b 0%, #fbbf24 100%); padding: 30px; text-align: center; border-radius: 12px 12px 0 0;"><h1 style="color: white; margin: 0; font-size: 24px;">💸 KDX商城</h1><p style="color: rgba(255,255,255,0.9); margin: 10px 0 0 0;">退款处理通知</p></div><div style="background: #f8f9fa; padding: 30px; border-radius: 0 0 12px 12px;"><div style="background: #fffbeb; padding: 20px; border-left: 4px solid #f59e0b; border-radius: 8px; margin-bottom: 25px;"><p style="color: #d97706; margin: 0; font-weight: 600;">您的退款申请已收到，我们正在处理中</p></div><div style="background: white; padding: 25px; border-radius: 10px;"><p style="margin: 0 0 15px 0; color: #333;"><strong>退款单号：</strong> <span style="color: #f59e0b;">' + (refund.id || '未知') + '</span></p><p style="margin: 0 0 15px 0; color: #333;"><strong>订单号：</strong> ' + (refund.orderId || '未知') + '</p><p style="margin: 0 0 15px 0; color: #333;"><strong>退款金额：</strong> <span style="color: #ef4444; font-size: 22px; font-weight: bold;">¥' + ((refund.amount || 0).toFixed(2)) + '</span></p><p style="margin: 0 0 15px 0; color: #333;"><strong>退款原因：</strong> ' + (refund.reason || '未说明') + '</p><p style="margin: 0 0 15px 0; color: #666; font-size: 14px;"><strong>申请时间：</strong> ' + new Date(refund.createdAt || Date.now()).toLocaleString('zh-CN') + '</p><p style="margin: 0; color: #666; font-size: 14px;"><strong>当前状态：</strong> <span style="color: #f59e0b; font-weight: 600;">' + (refund.status === 'pending' ? '等待处理' : '处理中') + '</span></p></div><div style="margin-top: 25px; text-align: center;"><p style="color: #666; font-size: 14px; margin: 0;">我们会尽快处理您的退款申请，处理完成后会再通知您</p><p style="color: #ccc; font-size: 12px; margin-top: 10px;">这是一封自动发送的邮件，请勿直接回复</p></div></div></div>';
...
module.exports = {
    sendVerificationCodeByEmail: sendVerificationCodeByEmail,
    ...
    SENDGRID_CONFIG: SENDGRID_CONFIG
};
